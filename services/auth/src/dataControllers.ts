...
const toPlotResponse = (plot: PlotDocument) => ({
...
const toFarmResponse = (farm: FarmDocument, plots: PlotDocument[]) => {
...
const toPartnerResponse = (partner: PartnerDocument) => ({
...
const calculateSummary = (orders: OrderDocument[]): Array<{
  id: string;
  label: string;
  value: string;
  helper?: string;
...
export const getFarms = async (req: AuthenticatedRequest, res: Response) => {
  ...
  const plotsByFarm = new Map<string, PlotDocument[]>();
...
export const linkFarm = async (req: AuthenticatedRequest, res: Response) => {
  const user = req.user;
  if (!user) {
    return res.status(401).json({ error: "Unauthorized" });
  }
  ...
  const plots = await Plot.find({ farmId: farm._id }).sort({ createdAt: 1 });
...
export const getPartners = async (req: AuthenticatedRequest, res: Response) => {
  ...
  const partners = await PartnerAccess.find({
    ownerRole: user.role,
    ownerIdentifier: user.identifier
  }).sort({ createdAt: 1 });
...
export const getOrders = async (req: AuthenticatedRequest, res: Response) => {
  ...
  const orders = await Order.find({ ownerRole: user.role, ownerIdentifier: user.identifier }).sort({
    dueDate: 1
  });
